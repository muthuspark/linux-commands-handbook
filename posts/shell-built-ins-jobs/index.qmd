...
Before we dive into `jobs`, let's quickly refresh our memory on background processes.  In Linux, you can run commands in the background by appending an ampersand (`&`) to the command. This allows you to continue using your shell while the command executes asynchronously.
